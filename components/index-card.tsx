...
export function IndexRow({ project }: { project: Project }) {
  ...
  const inner = (
    <>
      <span className="w-14 shrink-0 font-marginalia-sans text-[12px] text-mg-ink-faint">{project.year ?? "—"}</span>
      <span className="min-w-0 flex-1">
        <span className="group relative inline-block truncate font-marginalia-serif text-[16px] text-mg-ink">
          {project.name}
          <HandUnderline />
        </span>
        <span className="block truncate font-marginalia-sans text-[12.5px] text-mg-ink-faint">{project.oneLiner}</span>
      </span>
  ...
  const rowClass =
    "group relative flex items-center gap-4 border-b border-mg-line py-3.5 px-2 -mx-2 rounded-[1px] transition-colors hover:bg-mg-bg";
